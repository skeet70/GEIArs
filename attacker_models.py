...
def sequence_cross_entropy_with_logits(logits, targets, mask, label_smoothing, reduce):
    ...
    """
    print("inputs inputs.pt {} {}".format(label_smoothing, reduce))
    # save_file({"logits": logits}, "logits.st")
    # shape : (batch * sequence_length, num_classes)
    # get a flattened view of logits with an inferred number of rows and dimensions?
    logits_flat = logits.view(-1, logits.size(-1))
    # save_file({"logits_flat": logits_flat}, "logits_flat.st")
    # shape : (batch * sequence_length, num_classes)
...
def report_score(y_true, y_pred):
    ...
    print(
        "micro precision_score: {:.2f}".format(
            metrics.precision_score(y_true, y_pred, average="micro")
        )
    )
    print(
        "macro precision_score: {:.2f} ".format(
            metrics.precision_score(y_true, y_pred, average="macro")
        )
    )
    print("==" * 20)
    print(
        "micro recall_score: {:.2f}".format(
            metrics.recall_score(y_true, y_pred, average="micro")
        )
    )
    print(
        "macro recall_score: {:.2f} ".format(
            metrics.recall_score(y_true, y_pred, average="macro")
        )
    )
    print("==" * 20)
    print(
        "micro f1_score: {:.2f}".format(
            metrics.f1_score(y_true, y_pred, average="micro")
        )
    )
    print(
        "macro f1_score: {:.2f} ".format(
...
if __name__ == "__main__":
    ...
    inv_model.to(device)
    optimizer = torch.optim.Adam(inv_model.parameters(), lr=3e-5, eps=1e-06)
