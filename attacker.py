...
def process_data_test_simcse(
    data, batch_size, device, config, proj_dir=None, need_proj=False
):
...
def train_on_batch(batch_X, batch_D, model, tokenizer, criterion, device, train=True):
    ...
    print(
        "batch {} :::: input emb {}".format(batch_X_unsqueeze.size(), input_emb.size())
    )
    ...
    # attempt to get everything but the loss tensor off cuda memory so backward prop has enough memory
    del target, target_mask, logits
    gc.collect()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Create a 1280d projection of a 768d embedding. Use --no-projection to disable (if you get errors about mismatched dimension sizes).",
    )
    ...
    if config["data_type"] == "train":
        process_data(
            ...
            device,
            config,
            need_proj=args.projection,
        )
    elif config["data_type"] == "test":
        if "simcse" in config["embed_model"]:
            process_data_test_simcse(
                sent_list, batch_size, device, config, proj_dir=None, need_proj=False
            )
        else:
